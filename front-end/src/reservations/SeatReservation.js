...
function SeatReservation({ tables }) {
    ...
    const submitHandler = (event) => {
        ...
        if (table.capacity < reservation.people) {
            ...
            updateTable(table.table_id, reservationId)
                .then(() => {
                    updateReservation(reservationId, "seated");
                })
                .then(() => {
    ...
    return (
        <main>
            <h1 className="mb-5">Seat Reservation</h1>
            <h4>{reservation.first_name} {reservation.last_name} party of {reservation.people}</h4>
